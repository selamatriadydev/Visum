...
function tambah($conn){
    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if(!empty($nm_pekerjaan)){
            $sql = "INSERT INTO pekerjaan (nm_pekerjaan) VALUES('".$nm_pekerjaan."')";
        ...
        <form action="" method="POST">
            <fieldset>
               ...
               <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nm_pekerjaan" id="inputEmail3" placeholder="Nama" required="required">
                  </div>
                </div>
                <br>
                <br>
    ...
    while($data = mysqli_fetch_array($query)){
        ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $thisURL; ?>&aksi=update&id=<?php echo $data['id_pekerjaan']; ?>&nm=<?php echo $data['nm_pekerjaan']; ?>" class="btn btn-primary">Ubah</a>
                </td>
                <td>
                    <a href="<?php echo $thisURL; ?>&aksi=delete&id=<?php echo $data['id_pekerjaan']; ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
        <?php
   $no++; }
...
function ubah($conn){
    ...
    if(isset($_POST['btn_ubah'])){
        ...
        if(!empty($id) && !empty($nm_pekerjaan) ){
            $perubahan = "nm_pekerjaan='".$nm_pekerjaan."'";
            $sql_update = "UPDATE pekerjaan SET ".$perubahan." WHERE id_pekerjaan=$id";
    ...
    if(isset($_GET['id'])){
        ...
        ?>
            ...
            <hr>
            <form action="" method="POST">
            <input type="hidden" name="id_pekerjaan" value="<?php echo $_GET['id'] ?>"/>
            <fieldset>
               ...
               <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Nama</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nm_pekerjaan" id="inputEmail3" placeholder="Nama" required="required" value="<?php echo $_GET['nm']; ?>">
                  </div>
                </div>
                <br><br>
...
function hapus($conn){
    if(isset($_GET['id']) && isset($_GET['aksi'])){
        $thisURL="vs.php?page=".$_GET['page']."";
        $id = $_GET['id'];
        $sql_hapus = "DELETE FROM pekerjaan WHERE id_pekerjaan=" . $id;
