...
function tambah($conn){
    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if(!empty($id) && !empty($id_pemeriksaan) && !empty($ket) ){
            $sql = "INSERT INTO `keadaanumum` (`id_jeniskeadaanumum`, `id_pemeriksaan`, `keterangan`) VALUES ('".$id."', '".$id_pemeriksaan."', '".$ket."')";
        ...
        <form action="" method="POST">
            <fieldset>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                     <div class="form-group">
                      <select class="form-control select2" style="width: 100%;" required="required" name="id_jeniskeadaanumum">
                        <option value="" selected="selected">Pilih Jenis</option>
                        <?php
                        global $conn;
                        $query_gas=mysqli_query($conn, "SELECT * from jeniskeadaanumum");
                        while ($row2=mysqli_fetch_array($query_gas)) {?>
                        <option value="<?php echo $row2['id_jeniskeadaanumum']; ?>" ><?php echo $row2['nm_jeniskeadaanumum']; ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
...
function tampil_data($conn){
    ...
    $idper=$_GET['vshmy'];
    $sql = "SELECT * FROM keadaanumum, jeniskeadaanumum WHERE keadaanumum.id_jeniskeadaanumum=jeniskeadaanumum.id_jeniskeadaanumum and keadaanumum.id_pemeriksaan=$idper";
    ...
    while($data = mysqli_fetch_array($query)){
        ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $thisURL; ?>&aksi=update&id=<?php echo $data['id_keadaanumum']; ?>&jenis=<?php echo $data['keterangan']; ?>" class="btn btn-primary">Ubah</a> |
                    <a href="<?php echo $thisURL; ?>&aksi=delete&id=<?php echo $data['id_keadaanumum']; ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
        <?php
   $no++; }
...
function ubah($conn){
    ...
    if(isset($_POST['btn_ubah'])){
        $thisURL="vs.php?page=".$_GET['page']."&vshmy=".$_GET['vshmy']."&nav=".$_GET['nav']."";
        $id = $_POST['id_keadaanumum'];
        $id_jeniskeadaanumum = $_POST['id_jeniskeadaanumum'];
        $ket = $_POST['keterangan'];
        
        if(!empty($id) && !empty($id_jeniskeadaanumum) && !empty($ket)){
            $perubahan = "id_jeniskeadaanumum='".$id_jeniskeadaanumum."', keterangan='".$ket."'";
            $sql_update = "UPDATE keadaanumum SET ".$perubahan." WHERE id_keadaanumum=$id";
                  ...
                  <div class="col-sm-10">
                     <div class="form-group">
                      <select class="form-control select2" style="width: 100%;" required="required" name="id_jeniskeadaanumum">
                        ...
                        <?php
                        $ambil_id=$_GET['id'];
                        $query_gas0=mysqli_query($conn, "SELECT id_jeniskeadaanumum from keadaanumum WHERE id_keadaanumum='$ambil_id'");
                        $row0=mysqli_fetch_array($query_gas0);
                        $las_id=$row0['id_jeniskeadaanumum'];
                        $query_gas=mysqli_query($conn, "SELECT * from jeniskeadaanumum");
                        while ($row2=mysqli_fetch_array($query_gas)) { 
                          $next_id=$row2['id_jeniskeadaanumum'];
                          ?>
                        <option value="<?php echo $row2['id_jeniskeadaanumum']; ?>" <?php if($las_id==$next_id) echo 'selected="selected"';  ?>><?php echo $row2['nm_jeniskeadaanumum']; ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
