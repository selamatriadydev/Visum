...
function tambah($conn){
    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if(!empty($id) && !empty($id_pemeriksaan) && !empty($ket) ){
            $sql = "INSERT INTO `benda` (`id_jenisbenda`, `id_pemeriksaan`, `keterangan`) VALUES ('".$id."', '".$id_pemeriksaan."', '".$ket."')";
...
function tampil_data($conn){
    ...
    $idper=$_GET['vshmy'];
    $sql = "SELECT * FROM benda, jenisbenda WHERE benda.id_jenisbenda=jenisbenda.id_jenisbenda and benda.id_pemeriksaan=$idper";
    ...
    while($data = mysqli_fetch_array($query)){
        ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $thisURL; ?>&aksi=update&id=<?php echo $data['id_benda']; ?>&jenis=<?php echo $data['keterangan']; ?>" class="btn btn-primary">Ubah</a> |
                    <a href="<?php echo $thisURL; ?>&aksi=delete&id=<?php echo $data['id_benda']; ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
        <?php
   $no++; }
...
function ubah($conn){
    ...
    if(isset($_POST['btn_ubah'])){
        $thisURL="vs.php?page=".$_GET['page']."&vshmy=".$_GET['vshmy']."&nav=".$_GET['nav']."";
        $id = $_POST['id_benda'];
        $id_jenisbenda = $_POST['id_jenisbenda'];
        $ket = $_POST['keterangan'];
        
        if(!empty($id) && !empty($id_jenisbenda) && !empty($ket)){
            $perubahan = "id_jenisbenda='".$id_jenisbenda."', keterangan='".$ket."'";
            $sql_update = "UPDATE benda SET ".$perubahan." WHERE id_benda=$id";
    ...
    if(isset($_GET['id'])){
        ...
        ?>
            ...
            <hr>
            <form action="" method="POST">
            <input type="hidden" name="id_benda" value="<?php echo $_GET['id'] ?>"/>
            <fieldset>
                  ...
                  <div class="col-sm-10">
                     <div class="form-group">
                      <select class="form-control select2" style="width: 100%;" required="required" name="id_jenisbenda">
                        ...
                        <?php
                        $ambil_id=$_GET['id'];
                        $query_gas0=mysqli_query($conn, "SELECT id_jenisbenda from benda WHERE id_benda='$ambil_id'");
                        $row0=mysqli_fetch_array($query_gas0);
                        $las_id=$row0['id_jenisbenda'];
                        $query_gas=mysqli_query($conn, "SELECT * from jenisbenda");
                        while ($row2=mysqli_fetch_array($query_gas)) { 
                          $next_id=$row2['id_jenisbenda'];
                          ?>
                        <option value="<?php echo $row2['id_jenisbenda']; ?>" <?php if($las_id==$next_id) echo 'selected="selected"';  ?>><?php echo $row2['nm_jenisbenda']; ?></option>
                        <?php } ?>
                      </select>
                    </div>
                  </div>
...
function hapus($conn){
    if(isset($_GET['id']) && isset($_GET['aksi'])){
        $thisURL="vs.php?page=".$_GET['page']."&vshmy=".$_GET['vshmy']."&nav=".$_GET['nav']."";
        $id = $_GET['id'];
        $sql_hapus = "DELETE FROM benda WHERE id_benda=" . $id;
