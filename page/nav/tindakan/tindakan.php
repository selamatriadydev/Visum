...
function tambah($conn){
    ...
    if (isset($_POST['btn_simpan'])){
        ...
        if( !empty($id_pemeriksaan) && !empty($ket) ){
            $sql = "INSERT INTO `tindakan` (`id_pemeriksaan`, `nm_tindakan`) VALUES ('".$id_pemeriksaan."', '".$ket."')";
...
function tampil_data($conn){
    $sql = "SELECT * FROM tindakan WHERE id_pemeriksaan=".$_GET['vshmy']."";
    ...
    while($data = mysqli_fetch_array($query)){
        ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $thisURL; ?>&aksi=update&id=<?php echo $data['id_tindakan']; ?>&ket=<?php echo $data['nm_tindakan']; ?>" class="btn btn-primary">Ubah</a> |
                    <a href="<?php echo $thisURL; ?>&aksi=delete&id=<?php echo $data['id_tindakan']; ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
        <?php
   $no++; }
...
function ubah($conn){
    ...
    if(isset($_POST['btn_ubah'])){
        ...
        if(!empty($id) &&  !empty($ket)){
            $perubahan = " nm_tindakan='".$ket."'";
            $sql_update = "UPDATE tindakan SET ".$perubahan." WHERE id_tindakan=$id";
    ...
    if(isset($_GET['id'])){
        ...
        ?>
            ...
            <hr>
            <form action="" method="POST">
            <input type="hidden" name="id_tindakan" value="<?php echo $_GET['id'] ?>"/>
            <fieldset>
...
function hapus($conn){
    if(isset($_GET['id']) && isset($_GET['aksi'])){
        $thisURL="vs.php?page=".$_GET['page']."&vshmy=".$_GET['vshmy']."&nav=".$_GET['nav']."";
        $id = $_GET['id'];
        $sql_hapus = "DELETE FROM tindakan WHERE id_tindakan=" . $id;
